...
function RadioPlayer() {
  ...
  return (
    <div style={{
      ...
      <style>{`
        @keyframes pulse { 0%,100%{opacity:.4;transform:scale(1)} 50%{opacity:1;transform:scale(1.01)} }
        @keyframes spin { from{transform:rotate(0deg)} to{transform:rotate(360deg)} }
      `}</style>
      <audio ref={audioRef} />
      <div style={{ display: "flex", alignItems: "center", gap: 10, marginBottom: 12 }}>
        <div style={{
          width: 42, height: 42, borderRadius: "50%", background: C.purple,
          display: "flex", alignItems: "center", justifyContent: "center", fontSize: 20,
          animation: playing ? "spin 4s linear infinite" : "none", flexShrink: 0,
        }}>🎵</div>
      ...
      <div style={{ display: "flex", gap: 10 }}>
        <button onClick={play} disabled={playing || loading} style={{
        ...
        <button onClick={pause} disabled={!playing && !loading} style={{
...
function Card({ children, style = {}, accent }) {
  return (
    <div style={{
      background: C.card, borderRadius: 12, padding: "10px 12px", marginBottom: 8,
      borderLeft: accent ? `2px solid ${accent}` : "none", ...style,
    }}>{children}</div>
  );
}
...
function Pill({ children, color = C.violet }) {
  return <span style={{ display: "inline-block", borderRadius: 20, fontSize: 10, padding: "2px 8px", marginRight: 4, marginBottom: 4, background: color + "30", color }}>{children}</span>;
}
function BtnPrimary({ children, onClick, style = {} }) {
  return <button onClick={onClick} style={{ width: "100%", padding: "10px", borderRadius: 10, background: C.purple, border: "none", color: C.white, fontSize: 13, fontWeight: 500, cursor: "pointer", marginTop: 6, ...style }}>{children}</button>;
}
function BtnOutline({ children, onClick, style = {} }) {
  return <button onClick={onClick} style={{ width: "100%", padding: "9px", borderRadius: 10, background: "transparent", border: `0.5px solid ${C.violet}`, color: C.violet, fontSize: 13, cursor: "pointer", marginTop: 6, ...style }}>{children}</button>;
}

/* ── Stream Deck ── */
const DECK_APPS = [
  { label: "Professor Griff", emoji: "🎙", sub: "Interview App", color: "#7F77DD", file: "InterviewConductor.html" },
  { label: "Timeout Sports", emoji: "🏆", sub: "Trivia App", color: "#FFD700", file: "TimeoutSportsTalk.html" },
  { label: "Radio Meltdown", emoji: "📻", sub: "Full Player", color: "#9B59F5", url: "https://fairefaretravels.github.io/radiomeltdownorg/" },
  { label: "Back Office", emoji: "🏢", sub: "STV Admin", color: "#4CAF50", url: "https://fairefaretravels.github.io/stv-backoffice/" },
];

function StreamDeck() {
  return (
    <div style={{ marginBottom: 14 }}>
      <div style={{ fontSize: 10, color: C.muted, letterSpacing: "0.1em", textTransform: "uppercase", marginBottom: 8, fontWeight: 600 }}>⚡ Stream Deck</div>
        ...
        {DECK_APPS.map((app) => (
          <button
            ...
            onClick={() => {
              ...
              // For local files, show an alert with instructions since browsers can't open local files directly
              alert(`Open "${app.file}" in your browser from the same folder as this app.\n\nTip: Keep all your .html files in one folder and bookmark them!`);
            }}
            style={{
          ...
          >
            ...
            <div style={{ fontSize: 12, fontWeight: 700, color: C.white, marginBottom: 2 }}>{app.label}</div>
            <div style={{ fontSize: 10, color: app.color }}>{app.sub}</div>
          </button>
        ))}
      </div>
    </div>
...
/* ── Camera screen — REAL camera via getUserMedia ── */
function CameraScreen() {
  ...
  function startRecording() {
    ...
    mr.onstop = () => {
      ...
      const name = `Clip_${new Date().toLocaleTimeString()}.webm`;
      setRecordings(r => [{ url, name, time: new Date().toLocaleTimeString() }, ...r]);
    };
    mr.start();
  ...
  function toggleLive() {
    if (live) {
      ...
      timerRef.current = setInterval(() => {
        setElapsed(s => s + 1);
        setViewers(v => Math.max(0, v + Math.floor(Math.random() * 5 - 1)));
      }, 1000);
    }
  }
  ...
  return (
    <div>
      ...
      {/* Viewfinder */}
      <div style={{ background: "#000", borderRadius: 12, overflow: "hidden", position: "relative", marginBottom: 10, border: `0.5px solid ${C.border}`, aspectRatio: "4/3", display: "flex", alignItems: "center", justifyContent: "center" }}>
        <video ref={videoRef} autoPlay playsInline muted style={{ width: "100%", height: "100%", objectFit: "cover", display: camActive ? "block" : "none" }} />
        {!camActive && (
          <div style={{ textAlign: "center" }}>
            {camError
              ? <div style={{ color: C.red, fontSize: 11, padding: "0 16px" }}>{camError}</div>
              : <div>
                  <div style={{ fontSize: 36, opacity: 0.3, marginBottom: 8 }}>📷</div>
                  <div style={{ color: C.muted, fontSize: 11 }}>Tap Start Camera</div>
                </div>
            }
          </div>
        )}
        ...
        {/* Overlays */}
        {(recording || live) && (
          <div style={{ position: "absolute", top: 8, right: 8, background: C.red, borderRadius: 4, padding: "2px 6px", fontSize: 9, color: C.white, fontWeight: 700 }}>
      ...
      <div style={{ display: "flex", gap: 8, marginBottom: 10 }}>
        {!camActive
          ? <BtnPrimary style={{ marginTop: 0 }} onClick={() => startCamera()}>Start Camera</BtnPrimary>
          : <>
              <button onClick={flipCamera} style={{ flex: 1, padding: 8, borderRadius: 10, border: `0.5px solid ${C.violet}`, background: "transparent", color: C.violet, fontSize: 11, fontWeight: 500, cursor: "pointer" }}>
                🔄 Flip
              </button>
              <button onClick={recording ? stopRecording : startRecording} disabled={live} style={{ flex: 1, padding: 8, borderRadius: 10, border: "none", background: recording ? "#9b1111" : C.red, color: C.white, fontSize: 11, fontWeight: 500, cursor: live ? "default" : "pointer", opacity: live ? 0.4 : 1 }}>
                {recording ? "⏹ Stop" : "⏺ Record"}
              </button>
              <button onClick={toggleLive} disabled={recording} style={{ flex: 1, padding: 8, borderRadius: 10, border: "none", background: live ? "#4a1188" : C.purple, color: C.white, fontSize: 11, fontWeight: 500, cursor: recording ? "default" : "pointer", opacity: recording ? 0.4 : 1 }}>
                {live ? "⏹ End" : "🔴 Live"}
              </button>
              <button onClick={stopCamera} style={{ padding: "8px 10px", borderRadius: 10, border: `0.5px solid #444`, background: "transparent", color: "#666", fontSize: 11, cursor: "pointer" }}>✕</button>
            </>
        }
      </div>
      ...
      {recordings.length > 0 && (
        <div style={{ marginTop: 8 }}>
          <Label>Saved clips ({recordings.length})</Label>
          {recordings.map((r, i) => (
            <Card key={i} style={{ display: "flex", alignItems: "center", gap: 10 }}>
              <span style={{ fontSize: 20 }}>🎬</span>
              <div style={{ flex: 1, minWidth: 0 }}>
                <div style={{ fontSize: 11, color: C.white, fontWeight: 500 }}>{r.name}</div>
                <div style={{ fontSize: 9, color: C.muted }}>{r.time}</div>
              </div>
              <a href={r.url} download={r.name} style={{ fontSize: 10, color: C.violet, textDecoration: "none", background: C.violet + "20", padding: "4px 8px", borderRadius: 6 }}>⬇ Save</a>
            </Card>
          ))}
        </div>
      )}
...
function FeedScreen() {
  ...
  function publish(id) { setPosts(ps => ps.map(p => p.id === id ? { ...p, draft: false, time: "Just now · Posted" } : p)); }
  function like(id) { setPosts(ps => ps.map(p => p.id === id ? { ...p, likes: p.likes + 1 } : p)); }
...
export default function App() {
  ...
  return (
    <div style={{ fontFamily: "system-ui, sans-serif", minHeight: "100vh", position: "relative", overflow: "hidden", background: C.bg }}>
      <img src="/dancer1.gif" alt="" aria-hidden="true" style={{ position: "fixed", bottom: 0, right: 0, height: "55vh", width: "auto", opacity: 0.18, pointerEvents: "none", zIndex: 0, mixBlendMode: "screen" }} />
      <img src="/dancer1.gif" alt="" aria-hidden="true" style={{ position: "fixed", bottom: 0, left: 0, height: "40vh", width: "auto", opacity: 0.10, pointerEvents: "none", zIndex: 0, transform: "scaleX(-1)", mixBlendMode: "screen" }} />
